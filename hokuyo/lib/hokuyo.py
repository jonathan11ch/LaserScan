...
class Hokuyo(object):
    ...
    def laser_on(self):
        return self.__short_command(Hokuyo.LASER_ON, check_response=False)


    def laser_off(self):
        return self.__short_command(Hokuyo.LASER_OFF, check_response=False)

    def reset(self):
        return self.__short_command(Hokuyo.RESET, check_response=False)
    ...
    def __get_and_parse_scan(self, cluster_count, start_step, stop_step):
        ...
        result = ''.join(result)
        #print("START %s" % (Hokuyo.START_STEP))
        i = 0
        ...
        for chunk in chunks(result, 3):
            ...
            Angulos.append(((Hokuyo.STEP_DEG * cluster_count * i) + start))
            Distancias.append(decode(chunk))
            An_DIS.append((-1*Angulos[i]+ 90,Distancias[i]))
            #distances[ ((Hokuyo.STEP_DEG * cluster_count * i))] = decode(chunk)
            i += 1
        return An_DIS
